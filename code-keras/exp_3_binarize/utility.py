...
def UNET_G(isize, nc_in=3, nc_out=3, ngf=64, fixed_input_size=True, use_batchnorm=True):
    ...
    #m_g_i = Lambda(lambda z: K.round(sigmoid(x=z)))(m_g_i)
    #m_g_j = Lambda(lambda z: K.round(sigmoid(x=z)))(m_g_j)
...
def cycle_variables(netG1):
    ...
    m_g_i = Lambda(lambda x: K.round(x[:, :, :, 3:4]))(fake_out)
    ...
    rec_m_g_i = Lambda(lambda x: K.round(x[:, :, :, 3:4]))(rec_output)
