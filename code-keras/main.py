...
# define a function which can return m_g,i, m_g_j

m_g_i, m_g_j = generator_vars(netG1)

generate_masks = K.function([real_A], [m_g_i, m_g_j])
...
loss_DA, loss_GA, loss_cycA = D_loss(netDA, real_A, fake_B, rec_A, m_g_i, m_g_j)
...
while gen_iterations < niter:
    ...
    m_g_i, m_g_j = generate_masks([A])

    # actual warping execution 
    for i in range(m_g_i.shape[0]):
        mask_warp[i,:,:] = warping(m_g_j[i,:,:], m_g_i[i,:,:])
