...
def UNET_G(isize, nc_in=3, nc_out=3, ngf=64, fixed_input_size=True, use_batchnorm=True):
    ...
    m_g_i = Activation("sigmoid", name='mask_i_sigmoid')(m_g_i)
    m_g_j = Activation("sigmoid", name='mask_j_sigmoid')(m_g_j)
...
def cycle_variables(netG1):
    ...
    # compute the fake image, borrow body parts from the original image
    fake_im = m_g_i * im_i_j + (1 - m_g_i) * im_i
...
def D_loss(netD, real, fake, rec):
    ...
    pred_1 = netD(concatenate([im_i, im_i*m_g_i], axis = -1))  # positive
    pred_2 = netD(concatenate([im_i_j, im_j*m_g_j], axis = -1))  # negative, fake image
    pred_3 = netD(concatenate([im_i, im_j*m_g_j], axis = -1))  # negative, image and clothes mismatch
...
def read_image(data, idx_i):
    ...
    input_ = np.concatenate([input_i, input_j], axis=-1)
    assert input_.shape[-1] == 8
